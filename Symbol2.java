...
public class Symbol2 implements Jewels {
int point;
@Override
public ArrayList<String[]> shifting(ArrayList<String[]> mapNew, int k, int l, int o, int p, int u, int y) {
	...
	public ArrayList<String[]> deleting(ArrayList<String[]> map, int x, int y) {
		...
		symbols28.add("|");
			...
			if((y-1<=Main.columnCounter && y-1>=0 ) && (y-2<Main.columnCounter && y-2>0 ) && (symbols46.contains(map.get(x)[y-1])==true && symbols.contains(map.get(x)[y-2])==true)) {
	  			map.get(x)[y]=" ";
	  			map.get(x)[y-1]=" ";
	  			map.get(x)[y-2]=" ";
	  			this.point=60;
			...
			else if((y+1<Main.columnCounter && y+1>=0)  && (y+2<Main.columnCounter && y+2>=0)  && (symbols46.contains(map.get(x)[y+1])==true && symbols.contains(map.get(x)[y+2])==true) ){
	  			map.get(x)[y]=" ";
	  			map.get(x)[y+1]=" ";
	  			map.get(x)[y+2]=" ";
	  			this.point=60;
