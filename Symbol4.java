...
public class Symbol4 implements Jewels{
	@Override
	...
	public ArrayList<String[]> deleting(ArrayList<String[]> map, int x, int y) {
		...
		symbols28.add("|");
			...
			if((((x-1<=Main.rowCounter &&  y-1<=Main.columnCounter) && (x-1>=0 && y-1>=0)) && ((x-2<Main.rowCounter && y-2<Main.columnCounter) && (x-2>=0 && y-2>=0)))  && (symbols19.contains(map.get(x-1)[y-1])==true && symbols.contains(map.get(x-2)[y-2])==true)) { 
	  			map.get(x)[y]=" ";
	  			map.get(x-1)[y-1]=" ";
	  			map.get(x-2)[y-2]=" ";
	  			this.point=60;
			...
			else if((((x+1<=Main.rowCounter &&  y+1<=Main.columnCounter) && (x+1>=0 && y+1>=0)) && ((x+2<Main.rowCounter && y+2<Main.columnCounter) && (x+2>=0 && y+2>=0)))  && (symbols19.contains(map.get(x+1)[y+1])==true && symbols.contains(map.get(x+2)[y+2])==true)) {
	  			map.get(x)[y]=" ";
	  			map.get(x+1)[y+1]=" ";
	  			map.get(x+2)[y+2]=" ";
	  			this.point=60;
